...
%Note this is a simple cutdown version for fiddling around with and doing
%things manually.  Created 3/1/22

% MBW simulation to assess the calibration errors in the EcoMedics system
% that has become a problem
% 9/27/2021 -10/11/2021
% GK Prisk
...
% First off get some basic numbers into the simulation
 ...
 DSP=0.3;
 DSP= VX('Fractional Deadspace ',DSP);
 NonDSP=1-DSP;
 
 FRC=3000;
 TV=1000;
FRC = VX('FRC in mL ',FRC);
TV = VX('TV in mL ',TV);
PctFRC1 =60;
PctTV1 = 50;
...
RR=12;
RR = VX('Respiratory rate (BPM) ',RR);
N2Air=78;
N2Air = VX('N2 percentage in air ',N2Air);
N2End=1;
N2End = VX('N2 percentage for end of test ',N2End);
InspN2 = 0;
InspCO2=0;
ExpireCO2 =5;
MeanN2=N2Air;
%and set up the 2 compartments
PctFRC1 = VX('Percentage of FRC to compartment 1 (the low SV compartment)',PctFRC1);
FRC1=FRC * PctFRC1/100;
FRC2=FRC - FRC1;
PctTV1 = VX('Percentage of TV to compartment 1 (the low SV compartment) ',PctTV1);
TV1 = TV * PctTV1/100;
TV2 = TV - TV1;
SV1 = TV1/FRC1;
SV2 = TV2/FRC2;
...
SeqParam=25;
SeqParam = VX('Sequencing -- [0,50] ',SeqParam);
for i=1:100
...
for i=1:100
    ...
    ExpN2(i) = (N21 * Flow1(i) + N22 * Flow2(i))/(Flow1(i)+Flow2(i));
    ExpCO2(i) = ExpireCO2;
...
Snadded=0.06;
Snadded = VX('Value of Sn1 due to DCDI (defines Sacin, normal value ~0.05)',Snadded);
...
%crap=VX('FRC (correct value)',calcFRC); % this is the internal value
%crap=VX('FRC (correct value)',DisplayFRC); % This allows for DSP
calcbadFRC = cumbadN2(badstop)/(N2Air-BxBbadN2(badstop))*100;
DisplaybadFRC=calcbadFRC/NonDSP;

%crap=VX('FRC (EcoMedics value)',calcbadFRC); % Internal value
%crap=VX('FRC (EcoMedics value)',DisplaybadFRC); % Allows for DSP
